...
mean_pop_states <- function(state_name) {
  ...
  dt <- data.frame(state = state_name, mean = overall_cases$estimate, total = sum(filtered$cases_new))
...
#ggplot(mean_states, aes( x = state, y = mean, fill = state)) + geom_bar(stat = "identity") + theme(legend.position = "none") + geom_text(aes(label = round(mean, digits = 4), size = 0.5, hjust = -0.2))  + coord_flip() + labs(title = "Population mean of contracted cases by state", x = "State", y = "Population Mean")
...
ggplot(mean_states, aes( x = state, y = round((as.double(total)/population_states$pop * 100), digits = 2), fill = state)) + geom_bar(stat = "identity") + theme(legend.position = "none") + geom_text(aes(label = round((as.double(total)/population_states$pop * 100), digits = 2), size = 0.3, hjust = 1.5)) + coord_flip() + labs(title = "Cases per 100 population by state", x = "State", y = "Cases")
```
